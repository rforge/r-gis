distance.joint.trajectory.single.origin.xy <-
function(transition, id.xy, origin.xy, weights.raster) #transition is a sparse matrix. transition is a sparse matrix with as row/column names the cell numbers. Points of interest are locations (cellnumbers) for which we want to know mutual resistance distance. Procedure based on Fouss et al. (2007))
{
