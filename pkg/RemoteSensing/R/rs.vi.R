...
calc.arvi<-function(red,nir,blue) #ARVI: Atmospheric Resistant Vegetation Index
{
...
calc.dvi<-function(red, nir) #DVI: Difference Vegetation Index
{
	{
		result <- (nir - red)
	}
...
calc.evi<-function(blue, red, nir) #EVI: Enhanced Vegetation Index 
{
...
calc.gari<-function(red, nir, blue, green) #GARI: green atmospherically resistant vegetation index 
{
	tmp <-(nir - (green - (blue - red)))
	{
		result <- (nir - (green - (blue - red))) / tmp
	}
	return(result)
}


calc.gemi<-function(red,nir) #GEMI: Global Environmental Monitoring Index
{
	result <-
...
calc.gvi<-function(blue, green, red, nir, ch5, ch7) #Green Vegetation Index
{
	{
		result <- (-0.2848 * blue - 0.2435 * green - 0.5436 * red +
	     0.7243 * nir + 0.0840 * ch5 - 0.1800 * ch7)
	}
	return(result)
}


calc.ipvi<-function(red, nir) #IPVI: Infrared Percentage Vegetation Index
{
	{
		result <- (nir) / (nir + red)
	}
...
calc.msavi2<-function(red, nir) #MSAVI2: second Modified Soil Adjusted Vegetation Index
{
	{
...
calc.msavi<-function(red, nir) #MSAVI: Modified Soil Adjusted Vegetation Index
{
...
calc.ndvi<-function(red, nir) #NDVI: Normalized Difference Vegetation Index
{
...
calc.pvi<-function(red, nir) #PVI: Perpendicular Vegetation Index 
{
...
calc.savi<-function(red, nir) #SAVI: Soil Adjusted Vegetation Index
{
	{
...
calc.sr<-function(red, nir) #SR: Simple Vegetation ratio
{
	{
		result <- (nir / red)
	}
...
calc.wdvi<-function(red, nir, slslope = 1) #WDVI: Weighted Difference Vegetation Index
{
