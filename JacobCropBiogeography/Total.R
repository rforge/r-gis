...
world(ylim = c(-90, 90), xlim = c(0,90), add = TRUE, asp = 1, xlab = "", ylab = "", xaxt = "n", yaxt = "n", eps =	0.1, col=1,shift = FALSE, col.water="lightblue", col.land="lightgreen",	fill=T)
...
climate.tm <- transition.create(climate.rasterstack,transition.function="mahal",outer.meridian.connect=TRUE)
...
aez.tm <- transition.create(aez,transition.function=min,outer.meridian.connect=TRUE)
...
log.popdensity.tm <- transition.create(log.popdensity,transition.function=min,outer.meridian.connect=TRUE)

tm <- climate.tm * aez.tm * log.popdensity.tm

# Make a buffer around the suited area and discard other continents
adjacency.total <- adjacency.from.transition(tm) 
buffer <- adjacency.buffer.from.adjacency(tm,adjacency.total,3,outer.meridian.connect=TRUE)[,1:2]
tm@transitionmatrix[buffer] <- mean(tm@transitionmatrix@x[tm@transitionmatrix@x>0])-sd(tm@transitionmatrix@x[tm@transitionmatrix@x>0]) #a function for this is needed that does distance-based buffers
tm <- transition.solidify(tm)
tm <- transition.connected.component(tm,geodata)
...
time.depth.difference <- distance.from.level(time.depth[as.character(raster.get.cell.from.xy(tm,geodata[,2:3]))],mean)
