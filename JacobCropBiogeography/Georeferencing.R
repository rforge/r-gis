...
grf <- read.csv('Geo-referencing/WrongCountryFinalAllInst.csv')
colnames(grf)[5:10] <- c("Olat","Olon","BGID","BGlon","BGlat","BGunc")
grf.m <- as.matrix(grf[,c(6,5,8,9)])
errorCatDist <- error.report(grf.m, 3)
grf <- cbind(grf,errorCatDist)
write.csv(grf,'Geo-referencing/WrongCountryErrorReport2.csv',row.names=FALSE)

PartFinal <- apply(Part,1,function(x){
			if(all(is.na(x))){" "
			} else if(any(is.na(x))){na.omit(x)
